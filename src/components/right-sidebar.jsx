...
const Rightsidebar = ({activeSubTopic,setActiveSubTopic,subtopics}) => { 
  ...
  return (
    <aside className="hidden py-7 top-14 xl:block xl:w-2xs xl:flex-none sticky h-[calc(100vh-3.5rem)] overflow-y-auto scrollbar-ghost pl-3">
      <p className="uppercase text-xs tracking-widest font-semibold text-blue-500 mb-3">On this page</p>
        ...
        <div key={index}>
          ...
          <ul className="space-y-2 border-l border-slate-800">
            {section.commands.map((command, commandIndex) => (
              <li key={commandIndex} className="text-slate-300 hover:text-white transition-colors border-l -ml-px px-4 border-slate-800 hover:border-slate-500">
                <code className="bg-blue-950 px-1 py-0.5 rounded text-sm">{command.code}</code> - {command.description}
              </li>
            ))}
          </ul>
        </div>
